...
@interface FVPSystemControlsMessage ()
...
+ (nullable FVPSystemControlsMessage *)nullableFromList:(NSArray *)list {
  return (list) ? [FVPSystemControlsMessage fromList:list] : nil;
}
- (NSArray *)toList {
  return @[
    @(self.textureId),
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    ...
    case 133: 
      return [FVPSystemControlsMessage fromList:[self readValue]];
    case 134: 
...
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[FVPCreateMessage class]]) {
    ...
    [self writeByte:132];
    [self writeValue:[value toList]];
  } else if ([value isKindOfClass:[FVPSystemControlsMessage class]]) {
    [self writeByte:133];
    [self writeValue:[value toList]];
...
void SetUpFVPAVFoundationVideoPlayerApi(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FVPAVFoundationVideoPlayerApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.video_player_avfoundation.AVFoundationVideoPlayerApi.setSystemControls"
        binaryMessenger:binaryMessenger
        codec:FVPAVFoundationVideoPlayerApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(setBackgroundControls:error:)], @"FVPAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(setBackgroundControls:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FVPSystemControlsMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        [api setBackgroundControls:arg_msg error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
